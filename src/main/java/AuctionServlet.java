	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
	{
		...
		if(rs.next())
		{
			...
			else if(amt>=(hbid+min))
			{
				...
				ResultSet rst1=stmt2.executeQuery("select CURDATE() from dual");
				rst1.next();
                ...
                ResultSet rst=stmt.executeQuery("select enddate-CURDATE() from item_master where itemid="+id); 
				if(rst.next())
				{
				...
				/*if((year >= year1))
				{
					if((month >= month1))
					{
					    ...
					    i=stmt1.executeUpdate("insert into bidding_info values('"+bidder+"',"+amt+","+id+",CURDATE(),1) ");
