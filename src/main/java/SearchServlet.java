...
public class SearchServlet extends HttpServlet
{
	...
	Vector v = new Vector();
	try{
	PreparedStatement ps = con.prepareStatement("select * from item_master where itemname like ?");
	ps.setString(1,"%"+s+"%");
	...
	ps.close();
	   if(id != null)
	   {
		PreparedStatement psmt = con.prepareStatement("select * from item_master where catid =? and itemname like ?");
