...
pw.println("<BODY bgColor=#dbfcff leftMargin=0 topMargin=0 MARGINHEIGHT=0 MARGINWIDTH=0><!-- ImageReady Slices (Untitled-1) -->");
		...
		ResultSet rs2=st2.executeQuery("select * from item_master where enddate=CURDATE() ");
		...
		pw.println("<h2><center>Closing ITems Tomarrow </h2><br>");
		Statement st4=con.createStatement();
		ResultSet rs4=st4.executeQuery("select * from item_master where enddate=CURDATE()+1 ");
		pw.println("<center><table border=3 bordercolor=green>");
		pw.println("<thead><th>Item<Th>Description<Th>Current Price");
		pw.println("<Th>Number Of Bids<Th>Auction Starting Date</thead>");
		while(rs4.next())
